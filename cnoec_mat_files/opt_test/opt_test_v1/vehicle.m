function [zdot,F]=vehicle(t,z,u,d,th)
...
Ux      =       z(3,1);    % body x velocity (m/s)
beta    =       z(4,1);    % sideslip angle (rad)
psi     =       z(5,1);    % yaw angle (rad)
r       =       z(6,1);    % yaw rate (rad/s)
...
% Model equations
zdot(1,1)  =   Ux*cos(psi)-Uy*sin(psi);
zdot(2,1)  =   Ux*sin(psi)+Uy*cos(psi);
zdot(3,1)  =   (Fx-Fyf*sin(delta)-Fr-Fxd)/m;
zdot(4,1)  =   (Fyf*cos(delta)+Fyr+Fyd)/(m*Ux)-r;
zdot(5,1)  =   r;
zdot(6,1)  =   (a*Fyf*cos(delta)-b*Fyr)/Jz;
